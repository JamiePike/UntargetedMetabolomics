...
feature.nm.vec <- c("")
smpl.nm.vec <- c("F9.4","F12.3","C12.3")
grp.nm.vec <- c("First_Time-point","Other","Second-Time-point")
...
feature.nm.vec <- c("")
smpl.nm.vec <- c("F9.4","F12.3","C12.3","BLANK.2")
grp.nm.vec <- c("First_Time-point","Other","Second-Time-point")
mSet<-UpdateData(mSet, T)
mSet<-PreparePrenormData(mSet)
mSet<-Normalization(mSet, "CompNorm", "LogNorm", "ParetoNorm", "sodium_formate", ratio=FALSE, ratioNum=20)
...
mSet<-ANOVA.Anal(mSet, F, 0.44499, FALSE)
mSet<-PlotANOVA(mSet, "aov_2_", "png", 72, width=NA)
mSet<-PlotSubHeatMap(mSet, "heatmap_2_", "png", 72, width=NA, "norm", "row", "euclidean", "ward.D","bwm", 8,8, 10.0,0.02,10, 10, "tanova", 251, T, T, T, F, T, T, T,T,"overview")
